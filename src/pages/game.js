...
const GamePage = () => {
    ...
    useEffect(() => {
        const fetchStartAndEndPages = async () => {
            try {
                const fetchedPages = await getStartAndEndPagesByJoinCode(
                    joinCode
                );
            ...
            const interval = setInterval(async () => {
                try {
                    ...
                    if (status === "ended") {
                        ...
                        setPlayerPaths(
                            await getVisitedLinksByJoinCode(joinCode)
                        );
                        setGameStatus("ended");
                    }
                } catch (error) {
